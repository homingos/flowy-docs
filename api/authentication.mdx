...
The Flowy API authenticates with **API keys**. A key belongs to a workspace, and every run you start with it is billed to that workspace. Send the key as a Bearer token in the `Authorization` header on every request:
...
<Steps>
  ...
  <Step title="Choose the key's shape">
    Pick a **type** (Secret or Publishable), an **owner** (You or Machine), and a **permission** level (Full, Read-only, or Restricted). For a publishable key, add the **domains** it may run from. Optionally set a daily spend cap and rate limit under advanced options.
  </Step>
  <Step title="Copy it now">
    Your key (`flowy_…` or `flowy_pk_…`) is shown **once**. Copy it and store it — a secret key in a secrets manager or server environment variable; a publishable key wherever your frontend reads its config.
  </Step>
</Steps>
...
await fetch(`${BASE}/apps/${appId}/runs`, {
  ...
  body: JSON.stringify({ inputs: { node_1: "a red bicycle" } }),
});
```

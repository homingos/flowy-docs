...
<Steps>
  ...
  <Step title="Start the run">
    `POST` to the flow's `runs` endpoint with an `inputs` **array**. Each entry's `node_id` is the input's `name` from the previous step. Set its value with **`asset_url`** for media inputs (a URL) or **`prompt`** for text inputs (the text itself); a variable input (`arity`) takes an **array** of values under that same key. You get a `runId` back right away.

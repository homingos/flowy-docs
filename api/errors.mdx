...
<AccordionGroup>
  ...
  <Accordion title="400 — Bad request">
    ...
    **Solution:** Check the request body against the app's input schema from [Get an app](/api/get-app). `inputs` must be an array of entries, each with a `node_id` (the input's `name` from the schema) and a value — `asset_url` for media inputs or `prompt` for text inputs.
  </Accordion>
  ...
  <Accordion title="403 — Forbidden">
    **Problem:** The request is authenticated but not allowed. Common causes: the key lacks the required [scope](/api/permissions); the app is private and not in the key's workspace; the run belongs to another workspace; or a publishable key was used from a domain that isn't allowlisted.

    **Solution:** Grant the key the needed scope, use a key from the app's own workspace, or call from an allowed domain.
  </Accordion>

  <Accordion title="404 — Not found">
    **Problem:** No app or run matches the `appId` / `runId` in the path.

    **Solution:** Double-check the id. List apps with [List apps](/api/list-apps), and use the `runId` returned by [Start a run](/api/start-run).
  </Accordion>

 ...
 * video node on masai maara for the seedance retake — the two existing video
 * nodes have permanently stuck "Generating…" labels (client-disconnect quirk).
...
await page.keyboard.type(
  "Create a Video node named 'Dawn Drift' using the Default engine, placed in empty space far away from every other node (well below the whole graph): connect two similar wide savanna landscape images as first and last frames (image role: frames), one-line prompt about morning light drifting across the plain, duration 4 seconds, resolution 480p. Build and wire only — do NOT generate.",
  { delay: 4 },
);
...
const nodes = await page.evaluate(() =>
  Array.from(document.querySelectorAll(".react-flow__node"))
    ...
    .filter((n) => /Dawn Drift/i.test(n.text)),
);

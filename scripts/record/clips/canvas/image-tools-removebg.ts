...
const clip: Clip = {
  name: "canvas/image-tools-removebg",
  ...
  actions: async ({ page, h }) => {
    ...
    // Attach Remove Background — a connected tool node appears beside the image.
    await spawnTool(page, base, "Remove Background");
    await page.locator(".react-flow__node-toolBlock").first().waitFor({ state: "visible", timeout: 8000 });
    await h.beat(700);
    await page.keyboard.press("Shift+Digit1");
    await h.beat(1300);

    // Toggle the Refine edges control (cleans up hair / soft boundaries).
    const refine = page.getByRole("button", { name: /^(On|Off)$/ }).first();
    ...
    // Rest on the source → Remove Background pair, ready to run.

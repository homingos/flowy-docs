...
const clip: Clip = {
  ...
  actions: async ({ page, h }) => {
    ...
    // Attach Change Angle — the inspector shows a 3D orbit preview + 3 sliders.
    await spawnTool(page, base, "Change Angle");
    await page.locator(".react-flow__node-toolBlock").first().waitFor({ state: "visible", timeout: 8000 });
    await h.beat(900);

    // Drag the orbit turntable to swing the camera around the subject.
    const orbit = page.getByRole("img", { name: /orbit/i }).first();
    const ob = await orbit.boundingBox();
    if (ob) {
      const ox = ob.x + ob.width / 2;
      const oy = ob.y + ob.height / 2;
      await h.drag({ x: ox, y: oy }, { x: ox + 70, y: oy - 24 });
      await h.beat(500);
      await h.drag({ x: ox + 40, y: oy }, { x: ox - 60, y: oy + 18 });
      await h.beat(700);
    }

    // Nudge the Horizontal and Vertical angle sliders with the keyboard.
    const sliders = page.getByRole("slider");
    const horiz = sliders.nth(0);
    await h.click(horiz);
    for (let i = 0; i < 8; i++) { await page.keyboard.press("ArrowRight"); await h.beat(120); }
    await h.beat(600);
    const vert = sliders.nth(1);
    await h.click(vert);
    for (let i = 0; i < 5; i++) { await page.keyboard.press("ArrowUp"); await h.beat(120); }
    await h.beat(900);

    // Rest on the configured viewpoint (Generate angle would run the job).
    await h.moveTo({ x: 1133, y: 182 });
    await h.beat(1400);
  },
};
